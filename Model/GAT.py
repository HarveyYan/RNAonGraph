...
from lib.gat_utils import gat_model
...
class GAT:
    ...
    def _placeholders(self):
        ...
        self.bias_mat_ph = tf.placeholder(tf.float32, shape=[None, self.max_len, self.max_len])
        self.bias_mat_splits = tf.split(self.bias_mat_ph, len(self.gpu_device_list))
        self.inference_bias_mat_ph = tf.placeholder(tf.float32, shape=[None, self.max_len, self.max_len])
    ...
    def _build_gat(self, split_idx, mode):
        if mode == 'training':
            node_tensor = self.node_input_splits[split_idx]
            bias_tensor = self.bias_mat_splits[split_idx]
        elif mode == 'inference':
            node_tensor = self.inference_node
            bias_tensor = self.inference_bias_mat_ph
        else:
    ...
    def fit(self, X, y, epochs, batch_size, output_dir, dev_data=None, dev_targets=None, logging=False):
        ...
        if dev_rmd != 0:
            ...
            dev_bias_mat = dev_bias_mat[:-dev_rmd]
        ...
        for epoch in range(epochs):
            ...
            if train_rmd != 0:
                ...
                bias_mat = bias_mat[:-train_rmd]
                y = y[:-train_rmd]

            for i in range(iters_per_epoch):
                _node_tensor, _bias_mat, _labels \
                    = node_tensor[i * batch_size: (i + 1) * batch_size], \
                      bias_mat[i * batch_size: (i + 1) * batch_size], \
                      y[i * batch_size: (i + 1) * batch_size]

                self.sess.run(self.train_op,
                              feed_dict={self.node_input_ph: _node_tensor,
                                         self.bias_mat_ph: _bias_mat,
                                         self.labels: _labels,
                                         self.global_step: i,
            ...
            train_cost, train_acc, train_auc = \
                self.evaluate((node_tensor, bias_mat), y, batch_size)
            ...
            dev_cost, dev_acc, dev_auc = \
                self.evaluate((dev_node_tensor, dev_bias_mat), dev_targets, batch_size)
    ...
    def predict(self, X, batch_size, y=None):
        ...
        for i in range(iters):
            _node_tensor, _bias_mat = node_tensor[i * batch_size:(i + 1) * batch_size], \
                                      bias_mat[i * batch_size:(i + 1) * batch_size]
            feed_dict = {
                self.inference_node_ph: _node_tensor,
                self.inference_bias_mat_ph: _bias_mat,
                self.is_training_ph: False
            }
