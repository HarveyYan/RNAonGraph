...
from Model.Sparse_Mask_RGCN import SparseMaskRGCN
...
tf.app.flags.DEFINE_bool('return_label', True, '')
...
def run_one_rbp(idx, q):
    ...
    model = SparseMaskRGCN(dataset['VOCAB_VEC'].shape[1], len(lib.graphprot_dataloader.BOND_TYPE) - 1,
                           # excluding no bond
                           dataset['VOCAB_VEC'], device, FLAGS.return_label, **hp)
...
if __name__ == "__main__":
    ...
    shutil.copy(inspect.getfile(SparseMaskRGCN), backup_dir)
    ...
    dataset = \
        lib.graphprot_dataloader.load_clip_seq([TRAIN_RBP_ID], use_embedding=FLAGS.use_embedding,
                                               ...
                                               probabilistic=FLAGS.probabilistic, nucleotide_label=not FLAGS.return_label)[0]  # load one at a time
