...
tf.app.flags.DEFINE_bool('use_ghm', True, '')
...
def run_one_rbp(fold_idx, q):
    ...
    original_cost, original_acc, original_auc = \
        model.evaluate(original_test_data, original_dataset['label'][test_idx], BATCH_SIZE)
...
if __name__ == "__main__":
    ...
    if FLAGS.output_dir == '':
        output_dir = os.path.join('output', 'Joint-ada-sampling-debiased', cur_time)
    else:
        output_dir = os.path.join('output', 'Joint-ada-sampling-debiased', cur_time + '-' + FLAGS.output_dir)
    ...
    shutil.copy(inspect.getfile(JointAdaModel), backup_dir)
