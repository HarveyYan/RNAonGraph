...
from Model.Mask_RNATracker import MaskRNATracker
...
def run_one_rbp(idx, q):
    ...
    model = MaskRNATracker(dataset['VOCAB_VEC'].shape[1], dataset['VOCAB_VEC'], device, **hp)
...
if __name__ == "__main__":
    ...
    if FLAGS.output_dir == '':
        output_dir = os.path.join('output', 'RNATracker-Graphprot', cur_time)
    else:
        output_dir = os.path.join('output', 'RNATracker-Graphprot', cur_time + '-' + FLAGS.output_dir)
    ...
    shutil.copy(inspect.getfile(MaskRNATracker), backup_dir)
    ...
    dataset = \
        lib.graphprot_dataloader.load_clip_seq([TRAIN_RBP_ID], use_embedding=FLAGS.use_embedding, load_mat=False)[
